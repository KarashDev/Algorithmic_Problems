    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            void ReplaceNum(int[,] array, int i, int j, int y, int x, int entryNum, int newValue)
            {
                ...
                {
                    // Если замена самая первая - необходимо проверять окружающие числа не на newValue а на
                    // самого себя (array[y, x]).
                    // Если замена НЕ первая - необходимо проверять окружающие числа на newValue (на уже замененное число)

                    int numToReplace = entryNum;
                    bool isVeryFirstReplace = false;

                    if (i == y && j == x)
                    {
                        isVeryFirstReplace = true;
                    }

                    if (!isVeryFirstReplace)
                        numToReplace = newValue;

                    if (i == 0 && j == 0)
                    {
                        if (array[i + 1, j] == numToReplace || array[i, j + 1] == numToReplace)
                        {
                            array[i, j] = newValue;
                        }
                    }
                    ...
                    {
                        if (array[i - 1, j] == numToReplace || array[i + 1, j] == numToReplace || array[i, j - 1] == numToReplace || array[i, j + 1] == numToReplace)
                        {
                            array[i, j] = newValue;
                        }
                    }

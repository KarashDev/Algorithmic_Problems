	...
	class WorkSpace
	{
		static void Main(string[] args)
		{
			...
			int[] UpArray(int[] num)
			{
				...
				// Если длина больше 1 числа и числа разные за исключением добавленного нуля
				if (num == null || !num.Any() || num.Any(c => c.ToString().Length > 1 && c.ToString() != "10" && c.ToString() != "11")
					|| num.Any(c => c < 0) || num[0] == 1 && num[1] == 1)
					return null;
			...
			//var x = UpArray(new int[]{2, 3, 9});
			//foreach (int i in x)
			//{
			//             Console.Write(i);
			//}

			var x = UpArray(new int[] { 0, 0, 0, 2, 3, 9, 9, 9 });
			foreach (int i in x)
			{
				Console.Write(i);
			}
			...
			Console.WriteLine(IsSingleDigitInteger(10));

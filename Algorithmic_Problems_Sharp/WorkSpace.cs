	...
	class WorkSpace
	{
		static void Main(string[] args)
		{
			...
			int[] PartsSums(int[] ls)
			{
				...
				// Если операция первая - .Sum() и RemoveAt(0)
				// Если операция не первая - в outputList скопировать элемент но отнять от него удаляемый объект из nums
			...
			foreach (var item in PartsSums(new int[] { 1, 2, 3, 4, 5, 6 }))
			{
				Console.Write(item + " ");
			}
